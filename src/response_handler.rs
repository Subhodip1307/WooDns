...
impl ResponseBuilder {
    ...
    pub fn success_response(&self,request:Message,query:hickory_proto::op::query::Query,
    record:Record
    ...
    pub fn error_res(&self,request:Message,query:hickory_proto::op::query::Query)->anyhow::Result<Vec<u8>>{
        let mut response=self.build_response(request)?;
        response.add_query(query);
